...
function calc() {
    ...
    // общая ф-ция для подсчета 
    function calcTotal() {
        if (!sex || !height || !weight || !age || !ratio) {
            result.textContent = "Заполнитe";
            return;
        }
    ...
    function getDynamicInformation(selector) {
        ...
        input.addEventListener('input', () => {
            ...
            if (input.value.match(/\D/g)) { // находим не число (регулярное выражение)
...
dot.setAttribute('data-slide-to', i + 1); // установка для каждлой точки атрибут и будем устанавливать нумерацию
dot.style.cssText = `
...
//     showSlides(slideIndex); // инициализация кода ниже
//         // добавление 0 перед циврй
//     if (slides.length < 10) {
//         total.textContent = `0${slides.length}`;
//     } else { 
//         total.textContent = slides.length;
...
//         if (n > slides.length) { // если н больше чем кол-во слайдов
//             slideIndex = 1; // то перемещаемся в самое начало
//         }
...
//         slides.forEach(item => item.style.display = 'none'); //скрываем

//         slides[slideIndex - 1].style.display = 'block'; // показываем
...
//     function plusSlides(n) { // изменяет слайд индекс
//         showSlides(slideIndex += n)
//     }

//     prev.addEventListener('click', () => { // нажатие на стрелку назад
//         plusSlides(-1);
//     });

//     next.addEventListener('click', () => { // нажатие на стрелку вперед
//         plusSlides(1);
//     });
...
// создание табов на странице
  ...
  tabsParent.addEventListener('click', (event) => {
      ...
      if (target && target.classList.contains(tabsSelector.slice(1))) {
          tabs.forEach((item, i) => { //второ элемент это номер по порядку
              if (target == item) {
                  hideTabContent();
                  showTabContent(i);
              }
...
window.addEventListener('DOMContentLoaded', () => { // глобальный обработчик загрузки

    //вызов окна через определенное вермя
    const modalTimerId = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_1__.openModal)('.modal', modalTimerId), 500000); //установка времени появления
